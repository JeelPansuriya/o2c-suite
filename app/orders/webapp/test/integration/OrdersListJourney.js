...
sap.ui.define([
    ...
    function journey() {
        ...
        opaTest("Check filter bar", function (Given, When, Then) {
            Then.onTheOrdersList.onFilterBar().iCheckFilterField("orderID");
            Then.onTheOrdersList.onFilterBar().iCheckFilterField("customerID");
            Then.onTheOrdersList.onFilterBar().iCheckFilterField("creditStatus");
            Then.onTheOrdersList.onFilterBar().iCheckFilterField("orderStatus");
        });

        opaTest("Check table columns and actions", function (Given, When, Then) {
            Then.onTheOrdersList.onTable().iCheckColumns(6, {"orderID":{"header":"Order ID"},"customerID":{"header":"Customer ID"},"orderDate":{"header":"Order Date"},"totalAmount":{"header":"Total Amount"},"creditStatus":{"header":"Credit Status"},"orderStatus":{"header":"Order Status"}});
